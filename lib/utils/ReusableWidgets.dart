...
Widget appThemedTextField({
  ...
  // Defaults
  const TextStyle defaultHintStyle = const TextStyle(
...
Widget getAppThemedIconButton({title, onPress, IconData iconData}) {
  return InkWell(
    onTap: onPress,
    child: Card(
      clipBehavior: Clip.hardEdge,
...
Widget getCachedNetworkImage(
    {@required String url, BoxFit fit, height, width, Widget defaultImage}) {
  return new CachedNetworkImage(
    ...
    errorWidget: (BuildContext context, String error, Object obj) {
      return defaultImage ??
          new Image.asset(
            AssetStrings.defaultImage, //todo
            fit: BoxFit.cover,
//          height: 24.0,
          );
    },
...
getAppThemedAppBar(
  ...
  return AppBar(
    ...
    title: Text(
      titleText,
      style: TextStyle(
          fontWeight: FontWeight.bold, color: AppColors.appBlue, fontSize: 14),
    ),
    centerTitle: true,
